...
fn build_transaction(acc: AccountId) -> (InMsg, OutMsg, OutMsg) {
    ...
    transaction
        .write_in_msg(Some(&CommonMessage::Std(inmsg1.clone())))
        .unwrap();

    let inmsg_int = InMsg::Immediate(InMsgFinal::with_cells(
        ChildCell::with_cell(
            MsgEnvelope::with_message_and_fee(&inmsg1, 9u64.into())
                .unwrap()
                .serialize()
                .unwrap(),
        ),
    ...
    let out_msg1 = OutMsg::New(OutMsgNew::with_cells(
        ChildCell::with_cell(
            MsgEnvelope::with_message_and_fee(&outmsg1, 9u64.into())
                .unwrap()
                .serialize()
                .unwrap(),
        ),
...
fn test_blockbuilder_generate_blocks() {
    ...
    let mut block_builder =
        builder_with_shard_ident(ShardIdent::masterchain(), 1, BlkPrevInfo::default(), 0);
    ...
    if !builder_is_empty(&block_builder) {
        ...
        block_restored
            .read_from(&mut SliceData::load_cell(block_cells_restored).unwrap())
            .unwrap();
