...
use ton_block::{BlkPrevInfo, Serializable, ShardIdent, ShardStateUnsplit, HashmapAugType};
use ton_types::UInt256;
use crate::data::{BlocksStorage, FileBasedStorage, ShardStateStorage};
use crate::block::finality::generate_block_with_seq_no;

#[test]
fn test_create_directory_tree() {
    let mut path = PathBuf::from("../target");
    let _fbs = FileBasedStorage::with_path(ShardIdent::default(), path.clone()).unwrap();
...
fn test_save_and_restore_shard_state(){
    ...
    let fbs = FileBasedStorage::with_path(shard, PathBuf::from("../target")).unwrap();
    ...
    fbs.save_shard_state(&ss).unwrap();

    let ss_restored = fbs.shard_state().unwrap();
...
fn test_save_and_restore_blocks() {
    ...
    let fbs = FileBasedStorage::with_path(shard_ident.clone(), PathBuf::from("../target")).unwrap();
    ...
    for n in 0..10 {
        ...
        let mut count = 0;
        block.read_extra().unwrap().read_account_blocks().unwrap().iterate_objects(|acc_block| {
            count += acc_block.transaction_count()?;
            Ok(true)
        }).unwrap();
        println!("transaction count {}", count);
        println!("generate_block time elapsed sec={}.{} ", d.as_secs(), d.subsec_nanos());

        let now = Instant::now();

        fbs.save_block(&block, UInt256::ZERO).unwrap();
        let d = now.elapsed();
        println!("save_block time elapsed sec={}.{} ", d.as_secs(), d.subsec_nanos());

        let now = Instant::now();
        blocks.push(block);
        let d = now.elapsed();
        println!("blocks.push time elapsed sec={}.{} ", d.as_secs(), d.subsec_nanos());
    ...
    for n in 0..10 {
        ...
        let now = Instant::now();
        let block = fbs.block(n + 1, 0).unwrap();
        let d = now.elapsed();
        println!("get_block time elapsed sec={}.{} ", d.as_secs(), d.subsec_nanos());
