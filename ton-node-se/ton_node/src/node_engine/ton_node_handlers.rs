...
use poa::engines::{
    authority_round::subst::{ExecutedBlock, OrdinaryHeader, TONBlock, UnsignedBlock, H256},
    Engine, Seal,
};
...
pub fn init_ton_node_handlers(ton: &TonNodeEngine) {
    ...
    ton.register_timer(
        ...
        TonNodeEngine::gen_timer,
    );
    ton.register_timer(
        Duration::from_millis(ton.gen_block_timeout()),
        TonNodeEngine::reset_sync_timer,
    );
    ton.register_timer(
        Duration::from_millis(ton.gen_block_timer()),
    ...
    ton.register_response_callback(
        request.into_boxed(),
        TonNodeEngine::process_request_node_info,
    );
    ...
    */
    ...
    fn route_message_timer(&self, io: &dyn NetworkContext) {
        ...
        while let Some(msg) = self.message_queue.dequeue_out() {
            ...
            while next < vals.len() {
                ...
                debug!(
                    "TRY ROUTING {}/{} {}:{}:{}",
                    next, next_step, wc, addr, next_validator
                );
    ...
    fn send_message_to_node(
        ...
        self.send_request(
            ...
            peer,
            &request.into_boxed(),
            TonNodeEngine::process_send_message_result,
        )
    }
    ...
    fn send_block_to_validator(
        ...
        self.send_request(
            ...
            peer,
            &request.into_boxed(),
            TonNodeEngine::process_validate_confirm,
        )
    }

    fn send_empty_step_to_validator(
        &self,
        io: &dyn NetworkContext,
        peer: &PeerId,
        data: Vec<u8>,
    ...
    fn send_block_request(
        ...
        self.send_request(
            ...
            peer,
            &request.into_boxed(),
            TonNodeEngine::process_sync_block,
        )
    }
    ...
    fn verify_block(
        ...
        match request {
            NetworkProtocol::TonEngine_NetworkProtocol_ValidationRequest(request) => {
                ...
                let confirmation =
                    self.process_block(&Vec::from(request.signed_block.0), io, peer)?;
    ...
    fn process_validate_confirm(
        ...
        match request {
            ...
            NetworkProtocol::TonEngine_NetworkProtocol_Error(error) => {
                // validate and process block
                warn!(target: "node", "Validate block on client {} failure: {}", peer, error.msg);
            }
            _ => return node_err!(NodeErrorKind::TlIncompatiblePacketType),
        }
        Ok(())
    ...
    fn request_block(
        ...
        match request {
            NetworkProtocol::TonEngine_NetworkProtocol_RequestBlockByNumber(request) => {
                ...
                return Ok(self
                    .process_block_request(request.seq_no as u32, request.vert_seq_no as u32)?
                    .into_boxed());
            }
            _ => node_err!(NodeErrorKind::TlIncompatiblePacketType),
        }
    ...
    fn process_sync_block(
        ...
        match request {
            NetworkProtocol::TonEngine_NetworkProtocol_ResponceBlock(block) => {
                ...
                //self.process_confirmation(*confirmation, io, peer)?;
                if let Ok((mut seq_no, vert_seq_no)) =
                    self.process_block_sync(&block.signed_block.0)
                {
                    seq_no += 1;
                    if !self.incoming_blocks.exists(seq_no) {
                        if self
                            .send_block_request(io, peer, seq_no, vert_seq_no)
                            .is_err()
                        {
                            self.stop_sync();
                        }
...
impl TonNodeEngine {
    ...
    pub fn prepare_block(&self, timestamp: u32) -> NodeResult<BlockData> {
            ...
            if !self.is_synchronize.load(AtomicOrdering::SeqCst) {
                    ...
                    && !self.is_starting.load(AtomicOrdering::SeqCst)
                {
                        ...
                        )?
                    {
                        ...
                        let mut ca = ConfirmationAnalyzer::new(
        ...
        exc_block
            .header
            .set_number(block.read_info()?.seq_no() as u64);
        ...
        exc_block
            .header
            .set_parent_hash(H256::from(prev_ref_hash.as_slice()));
        ...
        let (block_data, seal_details) = match self
            .poa_context
            .lock()
            .engine
            .generate_seal(&exc_block, &parent)
        {
    ...
    fn process_empty_step(
        ...
        if ep.next_seq_no > expected_seq_no {
            warn!(target: "node", "Empty block with greater sequence number. Our node should synchronize.");
            self.request_for_synchronize(io, peer)?;
            return Ok(4);
        } else if ep.next_seq_no < expected_seq_no {
            warn!(target: "node", "Empty block with less sequence number. Other node should synchronize.");
            return Ok(5);
        }
    ...
    fn process_block(
            ...
            && !self.is_self_block_processing.load(AtomicOrdering::SeqCst)
        {
            if s_block.block().read_info()?.seq_no() > expected_seq_no {
                warn!(target: "node", "Block with greater sequence number. Our node should synchronize.");
                cc.result = 4;
                // need synchronize self
                self.request_for_synchronize(io, peer)?;
                return Ok(cc);
            } else if s_block.block().read_info()?.seq_no() < expected_seq_no {
                warn!(target: "node", "Block with less sequence number. Other node should synchronize.");
                cc.result = 5;
                return Ok(cc);
            }
            ...
            // is synchronize
            info!(target: "node", "Save block to incoming block cache");
    ...
    fn process_confirmation(
        &self,
        c: networkprotocol::ConfirmValidation,
        io: &dyn NetworkContext,
        _peer: &PeerId,
    ) -> NodeResult<()> {
        match c.result {
            ...
            1 => {
                warn!(target: "node", "process_confirmation result: Block verify check log::error!")
            }
            2 => {
                warn!(target: "node", "process_confirmation result: Apply block to ShardState failed!")
            }
    ...
    fn request_for_synchronize(&self, io: &dyn NetworkContext, peer: &PeerId) -> NodeResult<()> {
        if self.start_sync() {
            ...
            let (last_finalized_seq_no, _last_finalized_shard_hash) =
                self.finalizer.lock().get_last_finality_shard_hash()?;
    ...
    fn process_block_request(
        &self,
        seq_no: u32,
        vert_seq_no: u32,
    ) -> NodeResult<networkprotocol::ResponceBlock> {
        // get block with specified number and send to peer
        let block_data = self
            .finalizer
            .lock()
            .get_raw_block_by_seqno(seq_no, vert_seq_no);
    ...
    fn start_sync(&self) -> bool {
        let res = self.is_synchronize.compare_exchange(
            false,
            true,
    ...
    fn process_block_sync(&self, data: &[u8]) -> NodeResult<(u32, u32)> {
            ...
            && !self.is_self_block_processing.load(AtomicOrdering::SeqCst)
        {
            ...
            return if res.is_err() {
                ...
                node_err!(NodeErrorKind::SynchronizeError)
            } else {
                info!(target: "node", "!!! Received block SUCCESSFULLY APPLIED !!!");
                Ok((block_seq_no, block_vert_seq_no))
            };
        }
