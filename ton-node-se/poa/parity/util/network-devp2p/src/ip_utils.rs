...
pub trait SocketAddrExt {
	...
	fn is_benchmark(&self) -> bool { false }
...
impl SocketAddrExt for IpAddr {
	...
	fn is_reserve(&self) -> bool {
		match *self {
			IpAddr::V4(ref ip) => ip.is_reserve(),
			IpAddr::V6(ref ip) => ip.is_reserve(),
		}
	}
...
fn ipv4_benchmarking() {
	assert!(!Ipv4Addr::new(198, 17, 255, 255).is_benchmark());
	assert!(Ipv4Addr::new(198, 18, 0, 0).is_benchmark());
	assert!(Ipv4Addr::new(198, 19, 255, 255).is_benchmark());
	assert!(!Ipv4Addr::new(198, 20, 0, 0).is_benchmark());
}
