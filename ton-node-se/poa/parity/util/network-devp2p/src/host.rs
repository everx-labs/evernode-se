...
impl Host {
	...
	pub fn ready_peers(&self) -> Vec<PeerId> {
		...
		for i in (0..MAX_SESSIONS).map(|x| x + FIRST_SESSION) {
                    ...
                    if session.is_some() && session.unwrap().lock().is_ready() {
                        peers.push(i);
                    }
		}
		peers
	...
	fn connect_peers(&self, io: &IoContext<NetworkIoMessage>) {
		...
		debug!(target: "network", "Connecting peers: {} sessions, {} pending + {} started\r\n", egress_count + ingress_count, handshake_count, started);
	}
