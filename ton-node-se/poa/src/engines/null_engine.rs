...
use engines::authority_round::subst::{
    BlockNumber, Header, LiveBlock, TotalScoredHeader, U256, WithBalances, WithRewards
};
        ...
        <M::ExtendedHeader as TotalScoredHeader>::Value: Ord
{
	...
	fn machine(&self) -> &M { &self.machine }
		...
		let author = LiveBlock::header(&*block).author().clone();
		let number = LiveBlock::header(&*block).number();

		let reward = self.params.block_reward.clone();
		...
		// Bestow block reward
		let result_block_reward = reward.clone() + reward.clone().shr(5u64) * U256::from(n_uncles);
		rewards.push((author, RewardKind::Author, result_block_reward));

		// bestow uncle rewards.
		for u in LiveBlock::uncles(&*block) {
			let uncle_author = u.author();
			let result_uncle_reward = (reward.clone() * U256::from(8 + u.number() - number)).shr(3u64);
			rewards.push((uncle_author.clone(), RewardKind::uncle(number, u.number()), result_uncle_reward));
		}
...
/*
	fn snapshot_components(&self) -> Option<Box<::extension::snapshot::SnapshotComponents>> {
		Some(Box::new(::extension::snapshot::PowSnapshot::new(10000, 10000)))
	}
*/
