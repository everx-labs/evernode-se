...
use std::sync::atomic::{AtomicBool, AtomicU32, AtomicUsize, Ordering as AtomicOrdering, Ordering};
...
use engines::validator_set::{SimpleList, ValidatorSet};
use engines::{ConstructedVerifier, Engine, EngineError, Seal};
use error::{BlockError, PoaError, PoaErrorKind};
...
impl Step {
    ...
    fn duration_remaining(&self) -> Duration {
        let now = unix_now(self.delta.load(Ordering::Relaxed));
    ...
    fn calibrate(&self) {
        if self.calibrate {
            let new_step = unix_now(self.delta.load(Ordering::Relaxed)).as_secs() / (self.duration as u64);
...
impl EpochManager {
    ...
    fn zoom_to(
        ...
        let last_was_parent =
            self.finality_checker.subchain_head() == Some(header.parent_hash().clone());
        ...
        if (last_transition.block_hash != self.epoch_transition_hash)
            || (self.validators().count(header.parent_hash())
                != validators.count(header.parent_hash()))
        {
...
impl EmptyStep {
    fn from_sealed(
        sealed_empty_step: SealedEmptyStep,
        parent_hash: &H256,
        next_seq_no: u32,
    ) -> EmptyStep {
...
impl super::EpochVerifier<EthereumMachine> for EpochVerifier {
    ...
    fn check_finality_proof(&self, proof: &[u8]) -> Option<Vec<H256>> {
        ...
        {
            let mut push_header =
                |parent_header: &OrdinaryHeader, header: Option<&OrdinaryHeader>| {
                    ...
                    let newly_finalized: Vec<H256> = finality_checker
                        .push_hash(parent_header.hash().into(), signers)
                        .ok()?;
...
fn header_step(
    header: &OrdinaryHeader,
    _empty_steps_transition: u64,
) -> Result<u64, ::rlp::DecoderError> {
    /*
...
fn header_empty_steps(header: &OrdinaryHeader) -> Result<Vec<EmptyStep>, ::rlp::DecoderError> {
    ...
    Ok(empty_steps
        .into_iter()
        .map(|s| EmptyStep::from_sealed(s, header.parent_hash(), header.ton_block.get_seq_no()))
        .collect())
}
...
fn verify_external(
    ...
    let is_invalid_proposer = *header.author_key_id() != correct_proposer_key_id || {
        ...
        match &header.ton_block {
            TONBlock::Signed(x) => {
                !verify_signed_block(&correct_proposer, x, &header.ton_bytes, header.ton_offset)?
            }
...
impl AuthorityRound {
    ...
    pub fn new(
        ...
        let initial_step = our_params
            .start_step
            .unwrap_or_else(|| (unix_now(our_params.delta.load(Ordering::Relaxed)).as_secs() / (our_params.step_duration as u64)));
        let engine = Arc::new(AuthorityRound {
            transition_service: IoService::<()>::start()?,
            step: Arc::new(PermissionedStep {
                inner: Step {
                    delta: our_params.delta,
    ...
    pub fn set_validator(&self, index: usize) {
        ...
        self.validators
            .lock()
            .insert(pos, self.validators_all.get_key(index).clone());
    }
    ...
    fn epoch_set<'a>(
        &'a self,
        header: &OrdinaryHeader,
    ) -> Result<(CowLike<dyn ValidatorSet, SimpleList>, BlockNumber), PoaError> {
        Ok(
        ...
        if let (true, Some(me)) = (current_step > parent_step + 1, self.signer.read().key()) {
            ...
            for step in parent_step + 1..current_step {
                ...
                if skipped_primary != me.clone().into() {
                    ...
                    if !reported.insert(skipped_primary_key_id.clone()) {
                        break;
                    }
                    self.validators.lock().report_benign(
                        &skipped_primary_key_id,
                        set_number,
                        header.number(),
                    );
                }
    ...
    fn build_finality(
        ...
        ancestry: &mut dyn Iterator<Item = OrdinaryHeader>,
    ) -> Vec<H256> {
...
fn unix_now(delta: u32) -> Duration {
    UNIX_EPOCH
        .elapsed()
        .expect("Valid time has to be set in your system.")
        .add(Duration::from_secs(delta as u64))
}
...
impl Engine<EthereumMachine> for AuthorityRound {
    ...
    fn ton_handle_message(&self, rlp: &[u8]) -> Result<(EmptyStep, u64), EngineError> {
        ...
        if empty_step.verify(&*self.validators.lock()).unwrap_or(false) {
            ...
            validator_key = empty_step
                .get_validator_key(&*self.validators.lock())
                .expect("Get validator key empty step failed");
        } else {
        ...
        if is_step_proposer(
				...
				(empty_steps.len() < self.maximum_empty_steps)
            {
                    ...
                    .compare_and_swap(true, false, AtomicOrdering::SeqCst)
                {
                    ...
                    let fields = vec![encode(&step), Vec::new(), gen.0, gen.1];
            ...
            let signed_block = match header.ton_block {
                TONBlock::Unsigned(ref x) => {
                    match SignedBlock::with_block_and_key(x.block.clone(), &self.ton_key) {
                        Ok(signed_block) => {
                            let mut signed_block_bytes = Vec::new();
                            signed_block
                                .write_to(&mut signed_block_bytes)
                                .expect("error signed_block.write_to");
                            signed_block_bytes
                        }
                        _ => {
                            warn!(
    ...
    fn on_new_block(
        ...
        self.validators
            .lock()
            .on_epoch_begin(first, &header.into(), &mut call)
    }
    ...
    fn on_close_block(&self, block: &mut ExecutedBlock) -> Result<(), PoaError> {
        ...
        if block.header().number() >= self.empty_steps_transition {
            let empty_steps = if block.header().seal().is_empty() {
                ...
                let parent = client
                    .block_header(BlockId::Hash(block.header().parent_hash().clone()))
        ...
        let rewards: Vec<_> = match self.block_reward_contract {
            ...
            _ => beneficiaries
                .into_iter()
                .map(|(author, reward_kind)| (author, reward_kind, self.block_reward.clone()))
                .collect(),
        };
    ...
    fn verify_block_basic(&self, header: &OrdinaryHeader) -> Result<(), PoaError> {
            ...
            && *header.difficulty() >= U256::from(U128::max_value())
        {
            return Err(From::from(BlockError::DifficultyOutOfBounds(OutOfBounds {
                min: None,
                max: Some(U256::from(U128::max_value()).into()),
    ...
    fn verify_block_family(
            ...
            || (header.number() >= self.validate_step_transition && step <= parent_step)
        {
            ...
            self.validators.lock().report_malicious(
                header.author(),
                set_number,
                header.number(),
                Default::default(),
        ...
        if header.number() >= self.validate_score_transition {
            ...
            if header.difficulty() != &expected_difficulty {
                return Err(From::from(BlockError::InvalidDifficulty(Mismatch {
                    expected: expected_difficulty.into(),
                    found: header.difficulty().clone().into(),
                })));
            }
        }
    ...
    fn sign(&self, hash: H256) -> Result<Signature, PoaError> {
        Ok(Signature(self.ton_key.sign(&hash.0[..]).to_bytes()))
