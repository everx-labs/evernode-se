...
use self::authority_round::EmptyStep;
mod basic_authority;
...
pub use engines::authority_round::{AuthorityRound, AuthorityRoundParams};
pub use engines::basic_authority::{BasicAuthority, BasicAuthorityParams};
pub use engines::epoch::{EpochVerifier, PendingTransition, Transition as EpochTransition};
pub use engines::instant_seal::{InstantSeal, InstantSealParams};
pub use engines::null_engine::{NullEngine, NullEngineParams};
use error::PoaError;

use engines::authority_round::subst::{
    AccountProvider, ActionValue, Address, AncestryAction, BlockNumber, CallType, EthereumMachine, 
    ExecutedBlock, H256, LocalizedMachine, Machine, OrdinaryHeader, Password, 
    Signature, SignedTransaction, TotalScoredHeader, U256, UnverifiedTransaction
};
...
pub fn default_system_or_code_call<'a>(machine: &'a EthereumMachine, block: &'a mut ExecutedBlock) 
...
pub type PendingTransitionStore<'a> = dyn Fn(H256) -> Option<PendingTransition> + 'a;

/// Proof dependent on state.
pub trait StateDependentProof<M: Machine>: Send + Sync {
	/// Generate a proof, given the state.
	// TODO: make this into an &M::StateContext
	fn generate_proof<'a>(&self, state: &<M as LocalizedMachine<'a>>::StateContext) -> Result<Vec<u8>, String>;
...
impl<'a, M: Machine> ConstructedVerifier<'a, M> {
	...
	/// already.
	pub fn known_confirmed(self) -> Result<Box<dyn EpochVerifier<M>>, PoaError> {
		match self {
...
pub trait Engine<M: self::authority_round::subst::Machine>: Sync + Send {
	...
	fn generate_seal(&self, _block: &M::LiveBlock, _parent: &M::Header) -> (Seal, Vec<u128>) { 
            (Seal::None, Vec::new())
        }
	...
	/// Genesis epoch data.
	fn genesis_epoch_data<'a>(&self, _header: &M::Header, _state: &<M as LocalizedMachine<'a>>::StateContext) -> Result<Vec<u8>, String> { Ok(Vec::new()) }
	...
	fn signals_epoch_end<'a>(&self, _header: &M::Header, _aux: <M as LocalizedMachine<'a>>::AuxiliaryData)
		-> EpochChange<M>
	{
		EpochChange::No
	}
	...
	fn handle_message(&self, _message: &[u8]) -> Result<EmptyStep, EngineError> { Err(EngineError::UnexpectedMessage) }

	fn ton_handle_message(&self, _message: &[u8]) -> Result<(EmptyStep, u64), EngineError> { Err(EngineError::UnexpectedMessage) }
...
//		self.snapshot_components().is_some()
	...
	/// the ancestry exists.
	fn ancestry_actions(&self, _header: &M::Header, _ancestry: &mut dyn Iterator<Item=M::ExtendedHeader>) -> Vec<AncestryAction> {
		Vec::new()
	}
...
*/
	...
	fn verify_transaction_unordered(&self, t: UnverifiedTransaction, header: &OrdinaryHeader) -> Result<SignedTransaction, PoaError> {
	...
	fn verify_transaction_basic(&self, t: &UnverifiedTransaction, header: &OrdinaryHeader) -> Result<(), PoaError> {
...
/*
	...
	fn decode_transaction(&self, transaction: &[u8]) -> Result<UnverifiedTransaction, Error> {
...
impl<T> EthEngine for T where T: Engine<EthereumMachine> { }
