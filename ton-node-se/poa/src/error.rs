...
use engines::authority_round::subst::{
    Address, BlockNumber, H256, SignError, U256
};
use tvm::types::Exception;

//pub use extension::executed::{ExecutionError, CallError};
...
impl From<PoaError> for TransactionImportError {
...
error_chain! {
	types {
		PoaError, PoaErrorKind, PoaResultExt, PoaResult;
	}
...
//			Queue(QueueError, QueueErrorKind) #[doc = "Io channel queue error"];
	...
	foreign_links {
		...
		Rand(::rand::Error) #[doc = "Rand error"];
...
*/
		...
		#[doc = "Account Provider (Sign) error"]
		SignAccountProvider(err: SignError) {
			description("Accounts Provider error")
			display("Accounts Provider error {}", err)
		}

		#[doc = "TVM Exception"]
		TVMException(err: Exception) {
			description("TVM Exception")
			display("TVM Exception {}", err)
		}
	}
}
...
impl From<SignError> for PoaError {
	fn from(err: SignError) -> PoaError {
		PoaErrorKind::SignAccountProvider(err).into()
	}
}

impl From<Exception> for PoaError {
	fn from(err: Exception) -> PoaError {
		PoaErrorKind::TVMException(err).into()
	}
}
...
impl<E> From<Box<E>> for PoaError where PoaError: From<E> {
	fn from(err: Box<E>) -> PoaError {
		PoaError::from(*err)
	}
}
